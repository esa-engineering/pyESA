...
components = [
    ...
    CheckBox("use_leader", "Usa Leader Line", default=True),
...
try:
                ...
                if selected_category["name"] == "Rooms":
                    ...
                    if isinstance(location, DB.LocationPoint):
                        ...
                        tag = DB.IndependentTag.Create(
                            ...
                            use_leader,  # Usa la scelta dell'utente
                            DB.TagOrientation.Horizontal, 
                            point
                        )
                        
                        # Se l'utente vuole usare la leader line, imposta la lunghezza
                        if use_leader:
                            # Ottieni l'endpoint della leader line
                            leader_end = tag.LeaderEnd
                            
                            # Calcola un punto a una certa distanza
                            # Creiamo un vettore nella direzione Y (arbitrario)
                            direction = XYZ(0, 1, 0)
                            
                            # Calcola il nuovo punto finale della leader line
                            new_leader_end = point + direction.Multiply(leader_length_feet)
                            
                            # Imposta il nuovo endpoint
                            tag.LeaderEnd = new_leader_end
    ...
    if use_leader:
        print("Tag creati con leader line di lunghezza {} mm.".format(form.values["leader_length"]))
    else:
