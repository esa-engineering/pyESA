...
__title__   = "Tag All Elements\nin Multiple Views"
__doc__     = """Version = 2.3
Date    = 09.05.2025
________________________________________________________________
Tagga tutti gli elementi della categoria selezionata visibili nelle viste selezionate con il tag scelto
con possibilità di aggiungere una leader line di tipo Attached End e specificarne la lunghezza
________________________________________________________________
Author(s):
...
components = [
    ...
    Label("Lunghezza Leader Line (mm):"),
    TextBox("leader_length", Text="10"),
    Separator(),
    Button("OK")
]
...
try:
    # Converti il valore in millimetri a piedi (unità interna di Revit)
    leader_length_mm = float(form.values["leader_length"])
    leader_length_feet = leader_length_mm / 304.8
except ValueError:
    TaskDialog.Show("Errore", "Inserire un valore numerico valido per la lunghezza della leader line.")
    leader_length_feet = 10 / 304.8  # Valore di default 10mm
                ...
                if selected_category["name"] == "Rooms":
                    ...
                    if isinstance(location, DB.LocationPoint):
                        ...
                        if use_leader:
                            ...
                            direction = XYZ(0, 1, 0)
                            tag_head_position = point + direction.Multiply(leader_length_feet)
                            
                            try:
                                # Crea il tag con leader
                                tag = DB.IndependentTag.Create(
                                    doc, 
                                    tag_type.Id, 
                                    view.Id, 
                                    DB.Reference(element), 
                                    True,  # Usa sempre leader quando richiesta
                                    DB.TagOrientation.Horizontal, 
                                    tag_head_position  # Posizione del tag (non dell'elemento)
                                )
                                
                                # Imposta la condizione di fine della leader line a "Attached"
                                # Questo deve essere fatto prima di impostare il LeaderEnd
                                tag.LeaderEndCondition = DB.LeaderEndCondition.Attached
                                
                                # Imposta l'endpoint della leader line al punto dell'elemento
                                # Questo posizionerà il tag alla distanza desiderata dall'elemento
#                                tag.LeaderElbow = tag_head_position - direction.Multiply(leader_length_feet / 2)
                            except Exception as e:
                                print("Avviso: Impossibile impostare la lunghezza leader personalizzata: {}".format(str(e)))
                                print("Leader creata con lunghezza di default.")
                                
                                # Fallback: crea tag con leader standard
                                tag = DB.IndependentTag.Create(
                                    doc, 
                                    tag_type.Id, 
                                    view.Id, 
