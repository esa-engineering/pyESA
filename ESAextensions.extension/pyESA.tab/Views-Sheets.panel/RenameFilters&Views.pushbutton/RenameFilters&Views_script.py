...
if elements:
	components = [
		...
		Label('Replace'),
		TextBox('replace'),
		CheckBox('cb_number', 'Sheet Number (works only for Sheets)'),
		Separator(),
		Button('OK')
	]
		...
		with revit.Transaction('Rename Filters&Views'):
			try:
				for elem in elements:
					if form.values['cb_number'] and elem.Category.Name == 'Sheets':
						orig_name = elem.Parameter[DB.BuiltInParameter.SHEET_NUMBER].AsString()
						new_name = orig_name.replace(form.values['find'],form.values['replace']) if len(form.values['find'])>0 else orig_name
						revit.update.update_param_value(
							elem.Parameter[DB.BuiltInParameter.SHEET_NUMBER],
							'{0}{1}{2}'.format(form.values['prefix'],new_name,form.values['suffix'])
							)
					else:
