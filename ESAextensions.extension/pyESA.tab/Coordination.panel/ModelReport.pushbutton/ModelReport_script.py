...
def f_convert_units(doc,measure,um):
	...
	if int(doc.Application.VersionNumber) < 2022:
		...
		try:
			if um == 'm':
				conversion = DB.UnitUtils.ConvertFromInternalUnits(measure, DB.UnitTypeId.Meters)
			elif um == 'mq':
				conversion = DB.UnitUtils.ConvertFromInternalUnits(measure, DB.UnitTypeId.SquareMeters)
			elif um == 'mc':
				conversion = DB.UnitUtils.ConvertFromInternalUnits(measure, DB.UnitTypeId.CubicMeters)
			elif um == 'rad':
				conversion = DB.UnitUtils.ConvertFromInternalUnits(measure, DB.UnitTypeId.Degrees)
		except:	pass		
	return conversion
...
if csv_path:
	...
	if len(levels)>0:
		lev_elev = [round(f_convert_units(rvtdoc, lev.get_Parameter(BIP.LEVEL_ELEV).AsDouble(),'m'),3) for lev in levels]
	...
	site_location_values = [f_convert_units(rvtdoc, site_location.Elevation, 'm')]
	site_location_props = ('Elevation', 'Latitude', 'Longitude')
	for slp in site_location_props[1:]:
		site_location_values.append(f_convert_units(rvtdoc, getattr(site_location, slp), 'rad'))
	...
	survey_point_values = (
		f_convert_units(rvtdoc, survey_point_location.X, 'm'),
		f_convert_units(rvtdoc, survey_point_location.Y, 'm'),
		f_convert_units(rvtdoc, survey_point_location.Z, 'm')
		)
	...
	project_point_values = (
		f_convert_units(rvtdoc, project_point_location.X, 'm'),
		f_convert_units(rvtdoc, project_point_location.Y, 'm'),
		f_convert_units(rvtdoc, project_point_location.Z, 'm'),
		f_convert_units(rvtdoc, project_point_angle, 'rad'),
	)
