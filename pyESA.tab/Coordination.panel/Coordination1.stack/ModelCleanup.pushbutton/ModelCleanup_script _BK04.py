...
def f_param_check(elem, param, value):
	"""
	Return True if the value of the checked parameter matches
			...
			with forms.ProgressBar(title=rvt_file.split('\\')[-1]) as pb:
				...
				if sheets_param_name:
					if sheets_param_values:
						for sp_value in sheets_param_values:
							for elemid in sheets_all_set:
								if f_param_check(f_id_to_elem(temp_doc, elemid), sheets_param_name, sp_value):
				...
				if views_param_name:
					if views_param_values:
						for vp_value in views_param_values:
							for elemid in views_all_set:
								if f_param_check(f_id_to_elem(temp_doc, elemid), views_param_name, vp_value):
