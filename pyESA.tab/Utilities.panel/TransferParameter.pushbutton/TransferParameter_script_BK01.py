...
def f_set_par_value(p_read, p_write, conv, success):
	count_ok = 0
	count_no = 0
	exc_msg = success
	if p_read.HasValue:
		if p_read.StorageType == DB.StorageType.String:
			par_value = p_read.AsString()
			par_value_str = p_read.AsString()
		elif p_read.StorageType == DB.StorageType.ElementId:
			par_value = p_read.AsElementId()
			par_value_str = p_read.AsValueString()
		elif p_read.StorageType == DB.StorageType.Double:
			par_value = p_read.AsDouble()
			par_value_str = p_read.AsValueString()
		elif p_read.StorageType == DB.StorageType.Integer:
			par_value = p_read.AsInteger()
			par_value_str = p_read.AsValueString()
		try:
			if conv:
				p_write.Set(par_value_str)
				# p_write.Set.Overloads[[DB.StorageType.String]](par_value_str)
			else:
				p_write.Set(par_value)
			count_ok = 1
...
with revit.Transaction(name='Transfer Parameter', doc=doc):
	for elem in elements_filtered:
		...
		if par_read:
			if par_write:
				if par_text:
					if par_write.Definition.ParameterType != DB.ParameterType.Text:
						elems_skipped.append((output.linkify(elem.Id), elem.Category.Name, msg_storage_type))
						elems_count_no += 1
					else:
						set_par_value = f_set_par_value(par_read, par_write, par_text, msg_transfer)
