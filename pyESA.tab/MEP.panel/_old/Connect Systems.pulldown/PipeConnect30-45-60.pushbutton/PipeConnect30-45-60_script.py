# -*- coding: utf-8 -*-
__title__ = 'Pipe Connect 30°/45°/60°'
__doc__ =	"""Version = 1.0
Date = 08.05.2025
________________________________________________________________
Pipe slope: Default 90 degrees\n
Instruction:\n
- Select in order the pipes to connect.\n
- Press ESC to end.
\n
Differently from the Trim/Extend, it can also connect
pipes at different elevation and aligned each other.\n
When the selected pipes are not aligned, the tool will
create a new pipe so to connect the two closest extremity of the pipes.
...
def	connectPipesWithPipe(doc, p1, p2):
	...
	newPipe = Pipe.Create(doc, pipe1.GetTypeId(), levId, conn01, conn02)
	return	newPipe
...
def createPipe(doc, p1, pt1, pt2):
	...
	levId = p1.Parameter[DB.BuiltInParameter.RBS_START_LEVEL_PARAM].AsElementId()
	newPipe = Pipe.Create(doc, pipe1.GetTypeId(), levId, conn01, pt2)
	return	newPipe

## LINE CONNECTING PIPES
...
# INPUTS
slopeDef = forms.CommandSwitchWindow.show([30,45,60,90], message='Pipe connector slope [degrees]:')
...
with forms.ProgressBar(title='select Pipes to connect - press Esc to stop', cancellable=True) as pb:

	selected_pipes = []
	prog = 0
	pb.update_progress(prog, 100)
	for pipe in revit.get_picked_elements_by_category(BIC.OST_PipeCurves, "Select Pipe element"):
		prog += 50
		pb.update_progress(prog, 100)
		...
		if pipe == None:
			...
			prog = 0
			pipe1 = selected_pipes[0]
			pipe2 = selected_pipes[1]
			selected_pipes = []

			ln1 = pipe1.Location.Curve
			ln2 = pipe2.Location.Curve

			# EXTEND/TRIM FIRST PIPE
			# pts1 = (ln1.GetEndPoint(0), ln1.GetEndPoint(1))
			# pts2 = (ln2.GetEndPoint(0), ln2.GetEndPoint(1))
			# dists = []
			# for p1 in pts1:
			...
			# DEFINE VALUE FOR PIPE INCLINATION
			...
			with revit.Transaction('M4B - Connessione {}'.format(nr)):
				...
				if ln1.Direction.IsAlmostEqualTo(baseNewLine.Direction)\
					...
					pipe1.Location.Curve = baseNewLine
					doc.Delete(pipe2.Id)
					...
					if baseNewLine.GetEndPoint(0).IsAlmostEqualTo(ln1.GetEndPoint(0)):
						...
						baseNewLine = DB.Line.CreateBound(ln1.GetEndPoint(0),
									baseNewLine.GetEndPoint(1).Subtract(oppDir))
					pipe1.Location.Curve = baseNewLine
					pipe3 = connectPipesWithPipe(doc, pipe1, pipe2)
					# CONNECT
					placeElbow(pipe1, pipe3)
					placeElbow(pipe3, pipe2)

				else:
					# CREATE NEW PIPE
					pipe3 = createPipe(doc, pipe1, 
					...
					try:
						pipe4 = connectPipesWithPipe(doc, pipe3, pipe2)
						placeElbow(pipe3, pipe4)
						placeElbow(pipe4, pipe2)
					except:
						# in case pipe3 is at same elevation of pipe2
						placeElbow(pipe3, pipe2)
